        ...
        static PlanetDataRepository()
        {
            ...
            for (int i = 0; i < planetsData.GetLength(0); i++)
            {
                ...
                var planet = new Planet()
                {
                    ...
                    Mass = double.Parse(planetsData[i, index++]),
                    EquitorialDiameter = double.Parse(planetsData[i, index++]),
                    OrbitalPeriod = double.Parse(planetsData[i, index++]),
                    RotationPeriod = double.Parse(planetsData[i, index++]),
                    ConfirmedMoons = double.Parse(planetsData[i, index++]),
